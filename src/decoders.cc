...
class ArrayBuilder : public ColumnBuilder {
 ...
 public:
  ArrayBuilder(const SqlTypeInfo& info, const UserOptions&)
    ...
    Builder = std::make_unique<arrow::ListBuilder>(arrow::default_memory_pool(),
                                                   std::move(value_builder_->Builder));
...
class BoxBuilder : public ColumnBuilder {
 ...
 public:
  BoxBuilder(const SqlTypeInfo& info, const UserOptions&) {
    ...
    Builder = std::make_unique<arrow::StructBuilder>(type, arrow::default_memory_pool(),
                                                     field_builders_);
    ptr_ = (arrow::StructBuilder*)Builder.get();
  }
...
class GenericBuilder : public ColumnBuilder {
 private:
  BuilderT* ptr_;
 ...
 public:
  GenericBuilder(const SqlTypeInfo& info, const UserOptions&) {
    ...
    ptr_ = reinterpret_cast<BuilderT*>(Builder.get());
  }
...
template <class T>
std::shared_ptr<ColumnBuilder> create(const SqlTypeInfo& info,
