/**
 * @file   endianness.h
 * @brief  Convert Endianness of shorts, longs, long longs, regardless of
 * architecture/OS
 *
...
#elif (                                                                                \
    defined(__BYTE_ORDER__) && __BYTE_ORDER__ == __ORDER_LITTLE_ENDIAN__) || /* gcc */ \
...
#if !defined(__LITTLE_ENDIAN__) & !defined(__BIG_ENDIAN__)
#error                                                                                 \
    "UNKNOWN Platform / endianness. Configure endianness checks for this platform or set explicitly."
#endif

#if defined(bswap16) || defined(bswap32) || defined(bswap64) || defined(bswapf) ||     \
    defined(bswapd)
...
#if defined(_MSC_VER) // needs to be first because msvc doesn't short-circuit after
                      // failing defined(__has_builtin)
...
#define bswap64(x) __builtin_bswap64((x))
    ...
    __has_builtin(__builtin_bswap64) /* for clang; gcc 5 fails on this and &&          \
                                        shortcircuit fails; must be after GCC check */
#define bswap16(x) __builtin_bswap16((x))
#define bswap32(x) __builtin_bswap32((x))
#define bswap64(x) __builtin_bswap64((x))
#else
...
static inline float bswapf(float f)
{
    ...
    /* Problem: de-referencing float pointer as uint32_t breaks strict-aliasing rules
     * for C++ and C, even if it normally works uint32_t val =
     * bswap32(*(reinterpret_cast<const uint32_t *>(&f))); return
     * *(reinterpret_cast<float *>(&val));
     */
    // memcpy approach is guaranteed to work in C & C++ and fn calls should be optimized
    // out:
    uint32_t asInt;
    ...
    // union approach is guaranteed to work in C99 and later (but not in C++, though in
    // practice it normally will):
    union
    {
        uint32_t asInt;
        float asFloat;
