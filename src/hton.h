...
static inline uint16_t unpack_uint16(const char* buf) {
  uint16_t nx;
  memcpy(reinterpret_cast<char*>(&nx), buf, sizeof(uint16_t));
  return ntoh16(nx);
}
...
static inline uint32_t unpack_uint32(const char* buf) {
  uint32_t nx;
  memcpy(reinterpret_cast<char*>(&nx), buf, sizeof(uint32_t));
  return ntoh32(nx);
}
...
static inline uint64_t unpack_uint64(const char* buf) {
  uint64_t nx;
  memcpy(reinterpret_cast<char*>(&nx), buf, sizeof(uint64_t));
  return ntoh64(nx);
}
